...
async def create_dansal(
    ...
    try:
        ...
        dansal_dict = validated.dict()
        dansal_dict["userId"] = current_user["user_id"]
        result = await dansal_collection.insert_one(dansal_dict)
...
@dansal_router.get("/all", response_model=List[DansalEntry])
