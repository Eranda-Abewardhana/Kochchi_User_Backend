...
import { FiUser, FiPhone, FiMail, FiLogOut, FiEdit, FiEye, FiClock, FiCheckCircle, FiList } from 'react-icons/fi';
  ...
  useEffect(() => {
    const fetchUser = async () => {
      ...
      try {
        ...
        const adsRes = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/ads/my`, {
          headers: {
            'accept': 'application/json',
            'Authorization': `Bearer ${token}`,
          },
          ...
          canvas.toBlob((blob) => {
            if (blob) {
              ...
              previewReader.onload = (previewEv) => {
                setUserData((prev) => ({ ...prev, profile_pic: previewEv.target.result }));
              };
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-blue-50 via-white to-purple-50">
        <div className="bg-white/90 p-10 rounded-3xl shadow-2xl border border-blue-100 flex flex-col items-center gap-6 max-w-md">
          ...
          <p className="text-gray-600 text-center">Please log in to view your profile and manage your advertisements.</p>
          <button
    ...
    <div className="min-h-screen overflow-y-auto bg-gradient-to-br from-blue-50 via-white to-purple-50 py-10 px-2 sm:px-6 lg:px-12 mt-20">
      ...
      <div className="w-full max-w-4xl mx-auto mb-8 flex justify-center">
        <div className="flex rounded-xl bg-white/80 shadow border border-blue-100 overflow-hidden">
          <button
            className={`flex items-center gap-2 px-6 py-3 font-semibold text-base transition focus:outline-none ${activeTab === 'profile' ? 'bg-blue-600 text-white' : 'text-blue-700 hover:bg-blue-100'}`}
            onClick={() => setActiveTab('profile')}
          >
            <FiUser className="w-5 h-5" /> Profile
          </button>
          <button
            className={`flex items-center gap-2 px-6 py-3 font-semibold text-base transition focus:outline-none ${activeTab === 'ads' ? 'bg-blue-600 text-white' : 'text-blue-700 hover:bg-blue-100'}`}
          ...
          <div className="max-h-[50vh] overflow-y-auto">
            {userAds.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-10">
                ...
                <p className="text-base text-gray-500 font-medium">No advertisements here which you posted yet.</p>
              </div>
            ) : (
