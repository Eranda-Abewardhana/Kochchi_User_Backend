  ...
  useEffect(() => {
    const fetchAds = async () => {
      try {
        ...
        setAds(data.filter(
          (ad) => ad.business_category === 'Day-out Packages'
        ));
      } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gray-100 pt-24 py-8 px-2 sm:px-6 md:px-12 lg:px-32">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl sm:text-4xl font-extrabold text-center text-gray-900 mb-2 drop-shadow-lg">Day Outing Packages</h1>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-10 bg-white border border-gray-200 shadow-lg rounded-2xl px-6 py-4 max-w-2xl mx-auto">
          <label className="flex text-lg flex-col text-base font-medium text-gray-700 w-full max-w-xs">
            District
            <select
              className="mt-1 font-medium w-full min-w-[140px] max-w-xs rounded-lg border border-gray-300 bg-gray-50 px-2 py-2 shadow focus:outline-none focus:ring-2 focus:ring-blue-200 text-gray-800 text-sm"
              value={district}
        ...
        {loading && <Loading message="Loading day outing packages..." />}
        {error && <p className="text-center text-red-600 font-semibold py-8">{error}</p>}
        {!loading && !error && filteredAds.length === 0 && (
          <div className="flex flex-col items-center justify-center py-16">
            <p className="text-xl text-gray-500 font-semibold">There are no day outing packages advertised on this site.</p>
          </div>
        )}
