...
const Page = () => {
  ...
  useEffect(() => {
    const fetchAds = async () => {
      try {
        ...
        setAds(data); // Removed categorical filtering to show all ads
      } catch (err) {
  ...
  const filteredAds = ads.filter(ad => {
    ...
    if (category && ad.business_category !== category) return false;
    return true;
  });
  ...
  return (
    <div className="min-h-screen bg-gray-100 pt-24 py-8 px-2 sm:px-6 md:px-12 lg:px-32">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl sm:text-4xl font-extrabold text-center text-gray-900 mb-2 drop-shadow-lg">All Advertisements</h1>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-10 bg-white border border-gray-200 shadow-lg rounded-2xl px-6 py-4 max-w-3xl mx-auto">
          <label className="flex flex-col text-xs font-medium text-gray-700 w-full max-w-xs">
            Category
            <select
              className="mt-1 w-full min-w-[200px] max-w-xs rounded-xl border border-gray-300 bg-gray-50 px-4 py-3 shadow focus:outline-none focus:ring-2 focus:ring-blue-200 text-gray-800"
              value={category}
        ...
        {loading && <Loading message="Loading advertisements..." />}
        {error && <p className="text-center text-red-600 font-semibold py-8">{error}</p>}
        {!loading && !error && filteredAds.length === 0 && (
          <div className="flex flex-col items-center justify-center py-16">
            <p className="text-xl text-gray-500 font-semibold">No advertisements found.</p>
          </div>
        )}
